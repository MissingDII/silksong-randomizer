...
namespace Silkipelago.HarmonyPatches.Steam
{
    ...
    public static class SteamValidationPatch
    {
        ...
        public static bool Prefix(AppId_t unOwnAppID, ref bool __result)
        {
            try
            {
                BasePatch.Logger.LogDebugPatchIsRunning(nameof(SteamAPI), nameof(SteamAPI.RestartAppIfNecessary), nameof(SteamValidationPatch), nameof(Prefix));
            ...
            {
                BasePatch.Logger.LogErrorException(nameof(SteamValidationPatch), nameof(Prefix), ex);
